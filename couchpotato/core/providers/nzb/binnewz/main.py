...
class BinnewZ(NZBProvider):
    ...
    cat_ids = [
        ([6, 27], ['dvdrip']),
        ([6, 27], ['brrip']),
        ([39, 49], ['720p', '1080p', 'bd50']),
    ]
    
    def _search(self, movie, quality, results):
        nzbDownloaders = [BinSearch(), NZBClub()]
        ...
        arguments = tryUrlencode({
            'chkInit': '1',
            'edTitre': TitleStringReal,
            'chkTitre': 'on',
            'chkFichier': 'on',
            'chkCat': 'on',
            #'edYear': movie['library']['year'],
        })
        
        arguments += '&typeAutres%5B%5D=13'

        for x in self.getCatId(quality['identifier']):
            arguments += '&cats%5B%5D='+str(x)

        log.error("BINNEWZ"+ arguments)
        #data = 'chkInit=1&edTitre='+TitleStringReal+'&chkTitre=on&chkFichier=on&chkCat=on&cats%5B%5D='+cat1+'&cats%5B%5D='+cat2+'&edAge=&edYear='
        try:
            soup = BeautifulSoup(urllib2.urlopen(self.urls['search'], arguments))
        except Exception, e:
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if m:
                    name = m.group(1)
                    year = m.group(2)
                    log.error("#############""date 1 " + year + "vs" + str(movieyear))
                    if int(year) > movieyear+1:
                        continue
                ...
                for searchItem in searchItems:
                    log.error("###FOUND: " + searchItem)
                for searchItem in searchItems:
                    resultno=1
                    for downloader in nzbDownloaders:
                        
                        log.error("Searching for download : " + name + ", search string = "+ searchItem + " on " + downloader.__class__.__name__)
                        try:
                            ...
                            if binsearch_result:
                                ...
                                resultno=resultno+1
                                log.error("Found : " + searchItem + " on " + downloader.__class__.__name__)
                                if resultno==3:
                                    break
                        except Exception, e:
