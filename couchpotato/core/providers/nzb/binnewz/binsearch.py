...
class BinSearch(NZBDownloader):
    ...
    def search(self, filename, minSize, newsgroup=None):
        log.error("###BINSEARCH Searching for download : " + filename)
        if newsgroup != None:
            binSearchURLs = [tryUrlencode({'server' : 1, 'max': '250', 'adv_g' : newsgroup, 'q' : filename}), tryUrlencode({'server' : 2, 'max': '250', 'adv_g' : newsgroup, 'q' : filename})]
        else:
            binSearchURLs = [tryUrlencode({'server' : 1, 'max': '250', 'q' : filename}), tryUrlencode({'server' : 2, 'max': '250', 'q' : filename})]

        for suffixURL in binSearchURLs:
            binSearchURL = "http://binsearch.info/?adv_age=&" + suffixURL
            log.error("###BINSEARCH URL : " + binSearchURL)
            ...
            for elem in binSearchSoup.findAll(lambda tag: tag.name=='tr' and (tag.get('bgcolor') == '#FFFFFF' or tag.get('bgcolor') == '#F6F7FA') and 'size:' in tag.text):
                ...
                for checkbox in elem.findAll(lambda tag: tag.name=='input' and tag.get('type') == 'checkbox'):
                    ...
                    log.error("###BINSEARCH MINSIZE : " + str(sizeInMegs))
                    log.error("###BINSEARCH MINSIZE : " + checkbox.get('name'))
